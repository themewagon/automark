...
const isStaticDemo = import.meta.env.PUBLIC_STATIC_DEMO === "true";
...
export default function PricingCheckout({
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    if (isStaticDemo) {
      setError(
        "Checkout is disabled in this static preview. Configure the Stripe API routes after downloading the template.",
      );
      return;
    }
    if (!name || !email) {
    ...
    try {
      ...
      if (data.success && data.url) {
        ...
        setError(
          data.error || "Failed to initiate checkout. Please try again.",
        );
        setLoading(false);
      }
    } catch (err: any) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="bg-white dark:bg-dark p-8 rounded-3xl w-full max-w-md shadow-2xl relative">
            ...
            <p className="text-sm text-text mb-6">
              Please enter your details below so we can set up your account
              before payment.
            </p>
